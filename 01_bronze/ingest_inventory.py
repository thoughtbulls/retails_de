...
df = (spark.readStream \
    ...
    .option("cloudFiles.schemaLocation", "dbfs:/Volumes/data/raw/_schemas/retails/dev/inventory/") \
    .option("columnNameOfCorruptRecord", "_rescued_data") \
    .load("dbfs:/Volumes/data/raw/sales/raw_data/retail/dev/inventory/")
    )
...
(df.writeStream \
    .format("delta") \
    .option("checkpointLocation", "dbfs:/Volumes/data/raw/_checkpoints/retails/dev/bronze/inventory/") \
    .option("mode", "append") \
    .option("mergeSchema", "true") \
    .option("path", "s3://thoughtbulls-dp-uc-root-ap-south-1-8affd0fb/external_data/bronze/retails/inventory_raw") \
    .partitionBy("ingestion_dt") \
    .trigger(availableNow=True) \
    .toTable("retails.bronze.inventory_raw")
)

# COMMAND ----------

GRANTS = "ALTER TABLE retails.bronze.inventory_raw OWNER TO `dp-sales-engineers`"
